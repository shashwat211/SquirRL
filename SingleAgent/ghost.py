...
class Qnetwork():
    def __init__(self, h_size, state_space_n, state_vector_n, action_space_n):
        ...
        self.drop_rate = tf.placeholder(shape = None, dtype = tf.float32)
        ...
        #print(self.fc2)
        self.fc2 = tf.layers.dropout(self.fc2, self.drop_rate)
    ...
    def get_Q_table(self, sess, s, drop_rate = 1):
        Q = sess.run(self.Qout, feed_dict={self.vectorIn:[s], self.drop_rate:drop_rate})
        Q = np.reshape(Q, [-1])
        return Q
    ...
    def act_bayesian(self, sess, s, drop_rate = 0):
        Q = self.get_Q_table(sess, s, drop_rate)
        a = np.argmax(Q)
        return a
...
env = SM_env_with_stale(max_hidden_block = HIDDEN_BLOCK, attacker_fraction = ALPHA, follower_fraction = GAMMA, rule = rule, stale_rate=stale_rate, dev = DEV, know_alpha = know_alpha, random_interval=interval, random_process = random_process)
        ...
        while j < max_epLength: #If the agent takes longer than 200 moves to reach either of the blocks, end the trial.
            ...
            if (total_steps < pre_train_steps):
                ...
                if (exploration == "egreedy"):
                    ...
                    a = mainQN.act_bayesian(sess, s, e) # bayesian

            t = env.step(s, a, move = True)
            s1,r,d,_ = t
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps % (update_freq) == 0:
                    ...
                    Q1 = sess.run(mainQN.predict, feed_dict={mainQN.vectorIn: np.reshape(np.vstack(trainBatch[:, 3]), [-1, env._state_vector_n]), mainQN.drop_rate:0})
                    Q2 = sess.run(targetQN.Qout, feed_dict={targetQN.vectorIn: np.reshape(np.vstack(trainBatch[:, 3]), [-1, env._state_vector_n]), targetQN.drop_rate:0})
                    ...
                    _ = sess.run(mainQN.updateModel, \
                                 feed_dict={mainQN.vectorIn: np.reshape(np.vstack(trainBatch[:, 0]), [-1, env._state_vector_n]), mainQN.targetQ: targetQ,
                                            mainQN.actions: trainBatch[:, 1], mainQN.drop_rate:0})
